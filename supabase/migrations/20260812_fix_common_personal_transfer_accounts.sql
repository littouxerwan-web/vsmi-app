...
create or replace function public.common_person_first_name(u auth.users)
returns text
language sql
stable
security definer
...
as $$
  select initcap(coalesce(
    ...
    case
      when lower(coalesce(u.email,'')) like '%laure%' then 'Laure'
      when lower(coalesce(u.email,'')) like '%erwan%' then 'Erwan'
...
set search_path = public, auth
as $$
  select a.id, a.name, public.common_person_first_name(u) as owner_name
  from public.personal_accounts a
  join auth.users u on u.id=a.owner_id
  where exists (
    select 1 from auth.users me
    where me.id=auth.uid()
      and lower(public.common_person_first_name(me)) in ('erwan','laure')
  )
    and a.is_active=true
    and a.account_type='checking'
    and lower(public.common_person_first_name(u)) in ('erwan','laure')
...
begin
  ...
  select a.owner_id, public.common_person_first_name(u)
  ...
  if v_owner is null or lower(v_first) <> lower(trim(p_person_name)) then
    raise exception 'Compte courant non autorise';
  end if;
