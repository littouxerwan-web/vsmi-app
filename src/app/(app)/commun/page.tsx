...
function MovementEditor({m,categories}:{m:any;categories:any[]}){
 ...
 return <details className="mt-3">
  ...
  <form action={updateCommonMovement} className="mt-3 grid gap-2 sm:grid-cols-2">
   ...
   <select name="category_id" defaultValue={m.category_id??""} className="rounded-xl border px-3 py-2"><option value="">Sans catégorie</option>{categories.map(c=><option key={c.id} value={c.id}>{c.name} · {c.movement_type==="income"?"Crédit":"Débit"}</option>)}</select>
  ...
  {m.kind==="recurrence"?<details className="mt-3 rounded-xl border border-black/10 p-3">
   ...
   <form action={updateCommonRecurrenceSeries} className="mt-3 grid gap-2 sm:grid-cols-2">
    ...
    <S label="Catégorie" name="category_id" defaultValue={m.recurrence.category_id??""} options={[["","Sans catégorie"],...categories.map(c=>[c.id,`${c.name} · ${c.movement_type==="income"?"Crédit":"Débit"}`])]}/>
...
export default async function Page({searchParams}:{searchParams:Promise<any>}){
 ...
 const [{data:settings},{data:cats=[]},{data:snaps=[]},{data:movs=[]},{data:recs=[]},{data:overrides=[]},{data:exclusions=[]},{data:transferAccounts=[]}]=await Promise.all([supa.from("common_settings").select("*").eq("singleton",true).maybeSingle(),supa.from("common_categories").select("*").eq("is_active",true).order("name"),supa.from("common_balance_snapshots").select("*").order("snapshot_date",{ascending:false}),supa.from("common_movements").select("*").neq("status","cancelled").order("movement_date",{ascending:false}),supa.from("common_recurrences").select("*").eq("is_active",true).order("start_date"),supa.from("common_recurrence_overrides").select("*"),supa.from("common_recurrence_exclusions").select("*"),supa.rpc("common_transfer_accounts")]);
 ...
 {vue==="encours"?<div className="space-y-5 px-3 py-5 sm:px-5 lg:px-6">
  ...
  <div className="grid gap-4 lg:grid-cols-4">
   ...
   <details className="rounded-2xl border bg-white p-4"><summary className="cursor-pointer font-semibold">+ Virement interne</summary><CommonTransferForm accounts={(transferAccounts as any[]).map(a=>({id:String(a.account_id),name:String(a.account_name),owner_name:String(a.owner_name)}))} people={[String(st.person_1_name??"Erwan"),String(st.person_2_name??"Laure")]} action={createCommonPersonalTransfer} today={today}/></details>
 ...
 {vue==="budget"?<div className="space-y-5 px-3 py-5 sm:px-5 lg:px-6">
  ...
  <section className="rounded-2xl border bg-white"><div className="border-b p-4"><h2 className="font-semibold">Dépenses régulières d’un mois type</h2><p className="text-xs text-neutral-500">Cochée = prorata des revenus N-1. Non cochée = 50/50. Le simulateur utilise l’équivalent mensuel de toutes les récurrences enregistrées.</p></div>{expenses.length?expenses.map(r=><div key={r.id} className="flex items-center gap-3 border-b p-4 last:border-0"><form action={toggleCommonProrata}><input type="hidden" name="id" value={r.id}/><input type="hidden" name="value" value={String(!r.prorate_by_income)}/><button aria-label={r.prorate_by_income?"Passer cette dépense à 50/50":"Proratiser cette dépense selon les revenus"} data-checked={r.prorate_by_income?"true":"false"} className="common-ledger-check grid size-7 shrink-0 place-items-center rounded-md border-2">{r.prorate_by_income?<Check size={16}/>:null}</button></form><div className="min-w-0 flex-1"><p className="font-medium">{r.label}</p><p className="text-xs text-neutral-500">{r.prorate_by_income?`Prorata revenus · ${(p1*100).toFixed(2)} % / ${(p2*100).toFixed(2)} %`:"Répartition 50 / 50"}</p></div><b>{money(monthly(r))}/mois</b><form action={deleteCommonRecurrence.bind(null,r.id)}><button><Trash2 size={16}/></button></form></div>):<p className="p-4 text-sm text-neutral-500">Aucune dépense régulière enregistrée.</p>}</section>
