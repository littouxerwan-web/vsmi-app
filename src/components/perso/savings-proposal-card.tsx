...
type Props={sourceAccountId:string;destinationAccountId:string;sourceName:string;destinationName:string;sourceMonth:string;proposalDate:string|null;proposedAmount:number};

export async function SavingsProposalCard({sourceAccountId,destinationAccountId,sourceName,destinationName,sourceMonth,proposalDate,proposedAmount}:Props){
 ...
 return <section className={`mt-5 rounded-3xl border p-5 ${accepted?"border-emerald-300 bg-emerald-50":"border-violet-200 bg-violet-50"}`}>
  <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div><p className={`flex items-center gap-2 font-semibold ${accepted?"text-emerald-950":"text-violet-950"}`}><Sparkles size={18}/>{accepted?"Versement épargne accepté":"Versement épargne proposé"}</p><p className={`mt-1 text-sm ${accepted?"text-emerald-800":"text-violet-800"}`}>Le {dateLabel(transferDate)} · {sourceName} <ArrowRight className="inline" size={14}/> {destinationName}</p></div><p className={`text-2xl font-semibold ${accepted?"text-emerald-950":"text-violet-950"}`}>{money(amount)}</p></div>
  <p className={`mt-3 text-xs ${accepted?"text-emerald-800":"text-violet-800"}`}>{accepted?"Le débit et le crédit sont maintenant enregistrés en prévision à la date du revenu choisie.":"Montant recalculé automatiquement après chaque évolution des soldes, mouvements et budgets. Une modification manuelle reste valable jusqu’au prochain changement du calcul automatique."}</p>
