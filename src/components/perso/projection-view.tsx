...
export function ProjectionView({accounts,categories,snapshots,movements,recurrences,overrides=[],exclusions=[],photoPayments=[],photoDefaultAccountId=null,movementDefaultAccountId=null,urssafDefaultAccountId=null,urssafStates=[],savingsProposals=[],savingsBudgets=[],profiles=[],currentBalances={}}:{accounts:Account[];categories:Category[];snapshots:Snapshot[];movements:Movement[];recurrences:Recurrence[];overrides?:Override[];exclusions?:Exclusion[];photoPayments?:PhotoPayment[];photoDefaultAccountId?:string|null;movementDefaultAccountId?:string|null;urssafDefaultAccountId?:string|null;urssafStates?:UrssafState[];savingsProposals?:SavingsProposalDecision[];savingsBudgets?:SavingsBudgetAllocation[];profiles?:SavingsProfile[];currentBalances?:Record<string,number>}){
 ...
 const projectionData=useMemo(()=>{
  ...
  while(current<=horizon){
   ...
   // Les virements d'épargne sont appliqués de façon atomique afin qu'une sortie
   // d'épargne ne puisse jamais être supérieure au montant réellement mobilisable.
   // Un versement vers l'épargne est également plafonné à l'excédent du compte courant
   // au-dessus de sa limite basse.
   const savingsTransfersToday=virtualSavingsOps.filter(o=>o.movement_date===date&&o.movement_type==="transfer_out"&&o.savingsProposal);
   for(const o of savingsTransfersToday){
    const meta=o.savingsProposal!;
    let amount=Math.max(0,Number(o.amount));
    const sourceBalance=Number(balances.get(meta.sourceAccountId)??0);
    if(meta.kind==="use"){
     amount=Math.min(amount,mobilizableSavingsForAccount(sourceBalance,meta.sourceAccountId,savingsBudgets,SAVINGS_FLOOR));
    }else{
     const threshold=Math.max(0,Number(profiles.find(p=>p.sourceAccountId===meta.sourceAccountId)?.threshold??0));
     amount=Math.min(amount,Math.max(0,sourceBalance-threshold));
    }
    if(amount<=0.009)continue;
    balances.set(meta.sourceAccountId,sourceBalance-amount);
    balances.set(meta.destinationAccountId,Number(balances.get(meta.destinationAccountId)??0)+amount);
   }
   ...
   if(isMonthEnd){
    ...
    // Invariant 1 : un compte d'épargne ne peut jamais être négatif. Si une
    // projection historique a tenté de retirer plus que disponible, on annule
    // uniquement la partie impossible du transfert en la répercutant sur le
    // compte courant associé. Le patrimoine total reste donc cohérent.
    for(const savingsAccount of accounts.filter(a=>a.account_type==="savings")){
     const savingsBalance=Number(balances.get(savingsAccount.id)??0);
     if(savingsBalance>=0)continue;
     const deficit=-savingsBalance;
     balances.set(savingsAccount.id,0);
     const linkedProfile=profiles.find(p=>p.destinationAccountId===savingsAccount.id&&p.sourceAccountId);
     if(linkedProfile?.sourceAccountId)balances.set(linkedProfile.sourceAccountId,Number(balances.get(linkedProfile.sourceAccountId)??0)-deficit);
    }

    // Invariant 2 : à la clôture mensuelle, chaque compte courant est remonté à
    // sa limite basse tant que son épargne associée est réellement mobilisable.
    // Cette consolidation est propre à Projection ; En cours conserve sa logique
    // quotidienne J-2 / grâce 5 jours.
    for(const profile of profiles){
     ...
     const needed=Math.max(0,threshold-checkingBalance);
     if(needed<=0.009)continue;
     const savingsBalance=Number(balances.get(profile.destinationAccountId)??0);
     const available=mobilizableSavingsForAccount(savingsBalance,profile.destinationAccountId,savingsBudgets,SAVINGS_FLOOR);
     const used=Math.min(needed,available);
     if(used<=0.009)continue;
     balances.set(profile.destinationAccountId,savingsBalance-used);
     balances.set(profile.sourceAccountId,checkingBalance+used);
     const meta:SavingsProposalMeta={sourceAccountId:profile.destinationAccountId,destinationAccountId:profile.sourceAccountId,sourceMonth:monthKey,automaticAmount:used,status:"automatic",kind:"use"};
     const label=`Protection mensuelle du seuil · ${profile.label}`;
     floorProtectionOps.push(
      {id:`floor-${profile.id}-${monthKey}-out`,projected:true,account_id:profile.destinationAccountId,category_id:null,movement_type:"transfer_out",label,amount:used,movement_date:date,status:"planned",savingsProposal:meta},
      {id:`floor-${profile.id}-${monthKey}-in`,projected:true,account_id:profile.sourceAccountId,category_id:null,movement_type:"transfer_in",label,amount:used,movement_date:date,status:"planned",savingsProposal:meta},
     );
    }
   }
  ...
  return {points:Array.from(byMonth.values()),floorProtectionOps};
 ...
 const monthSummary=useMemo(()=>{
  const visible=monthOps.filter(o=>accountId==="all"||o.account_id===accountId);
  const expenses=visible.filter(o=>o.movement_type==="expense"&&o.status!=="completed").reduce((sum,o)=>sum+Number(o.amount),0)+monthBudgets.reduce((sum,b)=>sum+Number(b.remaining),0);
  const incomes=visible.filter(o=>o.movement_type==="income"&&o.status!=="completed").reduce((sum,o)=>sum+Number(o.amount),0);
  const savingsUsed=prioritySavingsOps.filter(o=>o.savingsProposal?.kind==="use").reduce((sum,o)=>sum+Number(o.amount),0);
  const savingsProposed=prioritySavingsOps.filter(o=>o.savingsProposal?.kind==="deposit").reduce((sum,o)=>sum+Number(o.amount),0);
  return {expenses,incomes,savingsUsed,savingsProposed};
 },[monthOps,monthBudgets,accountId,prioritySavingsOps]);
