...
export function MovementDeleteChoices({
  movementId = null,
  recurrenceId = null,
  occurrenceDate,
  projected = false,
    ...
    return (
      <div className="grid gap-2 border-t border-black/10 pt-3">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          >
            <button className="rounded-xl border border-red-200 bg-white px-3 py-2 text-xs font-medium text-red-700 hover:bg-red-50">
              Ce mois-ci uniquement
            </button>
          </form>

          <form action={deleteRecurrenceSeriesFrom.bind(null, recurrenceId, occurrenceDate)}>
            <button className="rounded-xl bg-red-700 px-3 py-2 text-xs font-medium text-white hover:bg-red-800">
              Toute la série
            </button>
          </form>
        </div>
