...
export function CategoryCreateForm({categories,accounts,today}:{categories:Category[];accounts:Account[];today:string}){
  ...
  return <form action={createCategory} className="grid gap-4 sm:grid-cols-2">
    ...
    {kind==="category"?<label><span className="mb-2 block text-sm font-medium">Type</span><select name="movement_type" defaultValue="expense" className={field}><option value="expense">Dépense</option><option value="income">Revenu</option></select></label>:<label><span className="mb-2 block text-sm font-medium">Type de budget</span><select name="movement_type" defaultValue="expense" className={field}><option value="expense">Débit</option><option value="income">Crédit</option></select></label>}
