...
export function CategoryCreateForm({categories,accounts,today}:{categories:Category[];accounts:Account[];today:string}){
  ...
  return <form action={createCategory} className="grid gap-4 sm:grid-cols-2">
    ...
    <label><span className="mb-2 block text-sm font-medium">Catégorie parente</span><select name="parent_id" className={field}><option value="">Aucune</option>{categories.filter(c=>!c.parent_id).map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></label>
