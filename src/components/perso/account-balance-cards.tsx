...
export function AccountBalanceCards({accounts,audits,operations,currentMonth,initialOrder=[]}:{accounts:Account[];audits:Audit[];operations:Operation[];currentMonth:string;initialOrder?:string[]}){
 ...
 return <>
  ...
  <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-3 lg:grid-cols-4 2xl:grid-cols-5">{sortedAccounts.map((a,index)=>{const audit=currentAudit;const current=Number(audit?.opening[a.id]??0);const beforeSavings=current+Number(audit?.credits[a.id]??0)-Number(audit?.debits[a.id]??0);const afterSavings=Number(audit?.closing[a.id]??beforeSavings);const Icon=a.account_type==="savings"?PiggyBank:a.account_type==="crypto"?Bitcoin:Landmark;const type=a.account_type==="savings"?"Épargne":a.account_type==="crypto"?"Crypto":"Compte courant";const accountColor=accountColors[a.id]??a.color??"#D2AE57";return <article key={a.id} className="relative min-h-[11.2rem] overflow-hidden rounded-[1.45rem] border p-3.5 shadow-[0_10px_28px_rgba(0,0,0,.12)] sm:min-h-[11.6rem] sm:p-4" style={{backgroundColor:`color-mix(in srgb, ${accountColor} 7%, #fff)`,borderColor:accountColor,color:"#1f201d"}}><div className="flex min-h-[3rem] items-start justify-between gap-2"><div className="min-w-0"><p className="line-clamp-2 text-[14px] font-semibold leading-[1.08] sm:text-[15px]">{a.name}</p><p className="mt-1.5 text-[9px] font-semibold uppercase tracking-[.16em] text-neutral-500">{type}</p></div><div className="flex shrink-0 gap-1"><button type="button" onClick={()=>{setOpenAccount(a.id);setMonth(currentMonth)}} title="Voir l’évolution graphique" aria-label={`Voir l’évolution graphique de ${a.name}`} className="grid size-8 place-items-center rounded-full border border-black/10 bg-white/80 text-neutral-600 transition hover:bg-white hover:text-black"><BarChart3 size={16}/></button><a href={`/perso?vue=finances&quick=debit&account_id=${a.id}`} title={`Ajouter un mouvement sur ${a.name}`} aria-label={`Ajouter un mouvement sur ${a.name}`} className="grid size-8 place-items-center rounded-full border border-black/10 bg-white/80 text-neutral-600 transition hover:bg-white hover:text-black"><Plus size={17}/></a></div></div><div className="mt-3 rounded-xl border p-2.5" style={{backgroundColor:`color-mix(in srgb, ${accountColor} 18%, #fff)`,borderColor:`color-mix(in srgb, ${accountColor} 48%, #ddd)`}}><div className="space-y-1.5"><div className="flex items-baseline justify-between gap-2"><span className="text-[9px] uppercase tracking-[.11em] text-neutral-500">Aujourd’hui</span><strong className={`whitespace-nowrap text-[11px] font-semibold tabular-nums sm:text-[13px] ${current<0?"text-red-700":""}`}>{money(current)}</strong></div><div className="flex items-baseline justify-between gap-2 border-t border-black/10 pt-1.5"><span className="text-[9px] uppercase tracking-[.11em] text-neutral-500">Fin de mois</span><strong className={`whitespace-nowrap text-[11px] font-semibold tabular-nums sm:text-[13px] ${beforeSavings<0?"text-red-700":""}`}>{money(beforeSavings)}</strong></div><div className="flex items-baseline justify-between gap-2 border-t border-black/10 pt-1.5"><span className="text-[9px] uppercase tracking-[.09em] text-neutral-500">Après épargne</span><strong className={`whitespace-nowrap text-[11px] font-semibold tabular-nums sm:text-[13px] ${afterSavings<0?"text-red-700":"text-emerald-700"}`}>{money(afterSavings)}</strong></div></div></div>{organizing?<div className="mt-2.5 border-t border-black/10 pt-2.5"><div className="mb-2 flex items-center justify-between"><span className="text-[9px] font-semibold uppercase tracking-[.12em] text-neutral-500">Position</span><div className="flex gap-1"><button type="button" onClick={()=>moveAccountCard(a.id,-1)} disabled={index===0||pending} className="grid size-7 place-items-center rounded-full border border-black/10 bg-white/80 disabled:opacity-30" aria-label={`Déplacer ${a.name} avant`}><ArrowLeft size={13}/></button><button type="button" onClick={()=>moveAccountCard(a.id,1)} disabled={index===sortedAccounts.length-1||pending} className="grid size-7 place-items-center rounded-full border border-black/10 bg-white/80 disabled:opacity-30" aria-label={`Déplacer ${a.name} après`}><ArrowRight size={13}/></button></div></div><div className="flex flex-wrap gap-1.5" aria-label={`Couleur de ${a.name}`}>{ACCOUNT_TONES.map(t=><button key={t.id} type="button" disabled={pending} onClick={()=>{setAccountColors(prev=>({...prev,[a.id]:t.hex}));startTransition(async()=>{await saveDashboardColors({[`personal:${a.id}`]:t.id});});}} className={`size-6 rounded-full border-2 ${(toneForHex(accountColor)===t.id)?"border-white ring-1 ring-black/50":"border-black/15"}`} style={{backgroundColor:t.hex}} aria-label={t.label} title={t.label}/>)}</div></div>:null}</article>})}</div>
