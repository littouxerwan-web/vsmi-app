...
export function AccountBalanceCards({accounts,audits,operations,currentMonth}:{accounts:Account[];audits:Audit[];operations:Operation[];currentMonth:string}){
 ...
 return <>
  ...
  <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-3 lg:grid-cols-4 2xl:grid-cols-5">{accounts.map(a=>{const audit=currentAudit;const current=Number(audit?.opening[a.id]??0);const beforeSavings=current+Number(audit?.credits[a.id]??0)-Number(audit?.debits[a.id]??0);const afterSavings=Number(audit?.closing[a.id]??beforeSavings);const Icon=a.account_type==="savings"?PiggyBank:a.account_type==="crypto"?Bitcoin:Landmark;const type=a.account_type==="savings"?"Épargne":a.account_type==="crypto"?"Crypto":"Compte courant";const accountColor=accountColors[a.id]??a.color??"#D2AE57";return <article key={a.id} className={`relative flex min-h-[11.2rem] flex-col overflow-hidden rounded-[1.45rem] border p-3.5 shadow-[0_10px_28px_rgba(0,0,0,.18)] sm:min-h-[11.6rem] sm:p-4 ${String(accountColor).toLowerCase()==="#111111"?"text-white":"text-[#20211e]"}`} style={{backgroundColor:accountColor,borderColor:accountColor}}><div className="flex min-h-[3rem] items-start justify-between gap-2"><div className="min-w-0"><p className="line-clamp-2 text-[14px] font-semibold leading-[1.08] sm:text-[15px]">{a.name}</p><p className="mt-1.5 text-[9px] font-semibold uppercase tracking-[.16em] opacity-55">{type}</p></div><button type="button" onClick={()=>{setOpenAccount(a.id);setMonth(currentMonth)}} title="Voir l’évolution graphique" aria-label={`Voir l’évolution graphique de ${a.name}`} className="grid size-8 shrink-0 place-items-center rounded-full border border-current/20 bg-white/12 backdrop-blur transition hover:bg-white/25"><BarChart3 size={16}/></button></div><div className="mt-auto space-y-1.5 pt-3"><div className="flex items-baseline justify-between gap-2"><span className="text-[9px] uppercase tracking-[.11em] opacity-55">Aujourd’hui</span><strong className={`whitespace-nowrap text-[11px] font-semibold tabular-nums sm:text-[13px] ${current<0?"text-red-500":""}`}>{money(current)}</strong></div><div className="flex items-baseline justify-between gap-2 border-t border-current/10 pt-1.5"><span className="text-[9px] uppercase tracking-[.11em] opacity-55">Fin de mois</span><strong className={`whitespace-nowrap text-[11px] font-semibold tabular-nums sm:text-[13px] ${beforeSavings<0?"text-red-500":""}`}>{money(beforeSavings)}</strong></div><div className="flex items-baseline justify-between gap-2 border-t border-current/10 pt-1.5"><span className="text-[9px] uppercase tracking-[.09em] opacity-55">Après épargne</span><strong className={`whitespace-nowrap text-[11px] font-semibold tabular-nums sm:text-[13px] ${afterSavings<0?"text-red-500":String(accountColor).toLowerCase()==="#111111"?"text-[#E3C97E]":""}`}>{money(afterSavings)}</strong></div></div>{organizing?<div className="mt-2.5 border-t border-current/10 pt-2.5"><div className="flex flex-wrap gap-1.5" aria-label={`Couleur de ${a.name}`}>{ACCOUNT_TONES.map(t=><button key={t.id} type="button" disabled={pending} onClick={()=>{setAccountColors(prev=>({...prev,[a.id]:t.hex}));startTransition(async()=>{await saveDashboardColors({[`personal:${a.id}`]:t.id});});}} className={`size-6 rounded-full border-2 ${(toneForHex(accountColor)===t.id)?"border-white ring-1 ring-black/50":"border-black/15"}`} style={{backgroundColor:t.hex}} aria-label={t.label} title={t.label}/>)}</div></div>:null}</article>})}</div>
