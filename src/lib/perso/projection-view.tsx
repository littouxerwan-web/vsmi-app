...
function projectionMonths(){const out:string[]=[];const d=new Date();d.setDate(1);d.setHours(12,0,0,0);for(let i=0;i<60;i++){out.push(iso(d).slice(0,7));d.setMonth(d.getMonth()+1);}return out;}
...
export function ProjectionView({accounts,categories,snapshots,movements,recurrences,overrides=[],exclusions=[],photoPayments=[],photoDefaultAccountId=null,movementDefaultAccountId=null,urssafDefaultAccountId=null,urssafStates=[],savingsProposals=[],savingsBudgets=[],profiles=[],currentBalances={}}:{accounts:Account[];categories:Category[];snapshots:Snapshot[];movements:Movement[];recurrences:Recurrence[];overrides?:Override[];exclusions?:Exclusion[];photoPayments?:PhotoPayment[];photoDefaultAccountId?:string|null;movementDefaultAccountId?:string|null;urssafDefaultAccountId?:string|null;urssafStates?:UrssafState[];savingsProposals?:SavingsProposalDecision[];savingsBudgets?:SavingsBudgetAllocation[];profiles?:SavingsProfile[];currentBalances?:Record<string,number>}){
 ...
 const projectionData=useMemo(()=>{
  ...
  while(current<=horizon){const date=iso(current),monthKey=date.slice(0,7);
   ...
   // Filet de sécurité de projection : le seuil configuré est une vraie limite basse.
   // Après TOUS les flux du jour (débits, crédits, budgets, récurrences et virements
   // d'épargne déjà calculés), on couvre encore un éventuel écart résiduel avec
   // l'épargne réellement mobilisable. Cela garantit qu'un compte courant ne reste
   // jamais sous son seuil alors que son épargne associée peut encore le protéger.
   for(const profile of profiles){
    if(!profile.sourceAccountId||!profile.destinationAccountId)continue;
    const sourceAccount=accounts.find(a=>a.id===profile.sourceAccountId);
    const savingsAccount=accounts.find(a=>a.id===profile.destinationAccountId);
    if(sourceAccount?.account_type!=="checking"||savingsAccount?.account_type!=="savings")continue;
    const threshold=Math.max(0,Number(profile.threshold??0));
    const checkingBalance=Number(balances.get(profile.sourceAccountId)??0);
    if(checkingBalance>=threshold-0.009)continue;
    const savingsBalance=Number(balances.get(profile.destinationAccountId)??0);
    const mobilizable=mobilizableSavingsForAccount(savingsBalance,profile.destinationAccountId,savingsBudgets,SAVINGS_FLOOR);
    const needed=Math.max(0,Math.round((threshold-checkingBalance)*100)/100);
    const amount=Math.min(needed,mobilizable);
    if(amount<=0.009)continue;
    balances.set(profile.sourceAccountId,Math.round((checkingBalance+amount)*100)/100);
    balances.set(profile.destinationAccountId,Math.round((savingsBalance-amount)*100)/100);
    const meta:SavingsProposalMeta={sourceAccountId:profile.destinationAccountId,destinationAccountId:profile.sourceAccountId,sourceMonth:monthKey,automaticAmount:amount,status:"automatic",kind:"use"};
    const label=`Protection du seuil · ${profile.label}`;
    floorProtectionOps.push(
     {id:`floor-${profile.id}-${date}-out`,projected:false,account_id:profile.destinationAccountId,category_id:null,movement_type:"transfer_out",label,amount,movement_date:date,status:"planned",savingsProposal:meta},
     {id:`floor-${profile.id}-${date}-in`,projected:false,account_id:profile.sourceAccountId,category_id:null,movement_type:"transfer_in",label,amount,movement_date:date,status:"planned",savingsProposal:meta},
    );
   }
   // Règles physiques finales de la projection :
   // 1) un compte d'épargne ne peut jamais devenir débiteur ;
   // 2) si un compte courant reste sous sa cible et qu'il existe encore de l'épargne
   //    réellement mobilisable, on utilise cette épargne avant de laisser apparaître
   //    un solde impossible. Le compte d'épargne associé est utilisé en priorité,
   //    puis les autres comptes d'épargne mobilisables en dernier recours.
   for(const savingsAccount of accounts.filter(a=>a.account_type==="savings")){
    const value=Number(balances.get(savingsAccount.id)??0);
    if(value<0)balances.set(savingsAccount.id,0);
   }
   for(const checkingAccount of accounts.filter(a=>a.account_type==="checking")){
    const profile=profiles.find(p=>p.sourceAccountId===checkingAccount.id);
    const target=Math.max(0,Number(profile?.threshold??0));
    let checkingBalance=Number(balances.get(checkingAccount.id)??0);
    if(checkingBalance>=target-0.009)continue;
    const preferred=profile?.destinationAccountId??null;
    const savingsOrder=accounts.filter(a=>a.account_type==="savings").sort((a,b)=>a.id===preferred?-1:b.id===preferred?1:0);
    for(const savingsAccount of savingsOrder){
     if(checkingBalance>=target-0.009)break;
     const savingsBalance=Math.max(0,Number(balances.get(savingsAccount.id)??0));
     const available=mobilizableSavingsForAccount(savingsBalance,savingsAccount.id,savingsBudgets,SAVINGS_FLOOR);
     const needed=Math.max(0,Math.round((target-checkingBalance)*100)/100);
     const amount=Math.min(needed,available);
     if(amount<=0.009)continue;
     checkingBalance=Math.round((checkingBalance+amount)*100)/100;
     balances.set(checkingAccount.id,checkingBalance);
     balances.set(savingsAccount.id,Math.max(0,Math.round((savingsBalance-amount)*100)/100));
     const meta:SavingsProposalMeta={sourceAccountId:savingsAccount.id,destinationAccountId:checkingAccount.id,sourceMonth:monthKey,automaticAmount:amount,status:"automatic",kind:"use"};
     const label=`Protection trésorerie · ${checkingAccount.name}`;
     floorProtectionOps.push(
      {id:`fallback-${checkingAccount.id}-${savingsAccount.id}-${date}-out`,projected:false,account_id:savingsAccount.id,category_id:null,movement_type:"transfer_out",label,amount,movement_date:date,status:"planned",savingsProposal:meta},
      {id:`fallback-${checkingAccount.id}-${savingsAccount.id}-${date}-in`,projected:false,account_id:checkingAccount.id,category_id:null,movement_type:"transfer_in",label,amount,movement_date:date,status:"planned",savingsProposal:meta},
     );
    }
   }
  ...
  return {points:result,floorProtectionOps};
 },[accounts,currentBalances,movements,recurrences,overrides,exclusions,photoPayments,photoDefaultAccountId,urssafDefaultAccountId,urssafStates,years,todayIso,budgetRoots,categoryById,movementDefaultAccountId,virtualSavingsOps,profiles,savingsBudgets]);
 ...
 const monthSummary=useMemo(()=>{
  ...
  const savingsUse=visible.filter(o=>o.savingsProposal?.kind==="use"&&o.movement_type==="transfer_in").reduce((s,o)=>s+Number(o.amount),0);
  const savingsProposal=visible.filter(o=>o.savingsProposal?.kind==="deposit"&&o.movement_type==="transfer_out").reduce((s,o)=>s+Number(o.amount),0);
  ...
  const expense=visible.filter(o=>!isCredit(o)&&!o.savingsProposal&&!["transfer","transfer_out"].includes(o.movement_type)).reduce((s,o)=>s+Number(o.amount),0)
    + monthBudgets.reduce((s,b)=>s+Number(b.remaining),0);
  return {income,expense,savingsUse,savingsProposal};
 },[monthOps,monthBudgets]);
 ...
 const simulationAnalysis=useMemo(()=>{
  ...
  const evaluate=(candidateDate:string)=>{
   ...
   const mobilisable=savingsId?Math.max(0,savingsBaseline-SAVINGS_FLOOR):0;
