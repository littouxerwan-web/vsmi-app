...
import { AlertTriangle, BarChart3, Camera, Check, Pencil, Search, ShoppingCart, SlidersHorizontal, Sparkles, Trash2, X } from "lucide-react";
...
export function ProjectionView({accounts,categories,snapshots,movements,recurrences,overrides=[],exclusions=[],photoPayments=[],photoDefaultAccountId=null,movementDefaultAccountId=null,urssafDefaultAccountId=null,urssafStates=[],savingsProposals=[],profiles=[],currentBalances={}}:{accounts:Account[];categories:Category[];snapshots:Snapshot[];movements:Movement[];recurrences:Recurrence[];overrides?:Override[];exclusions?:Exclusion[];photoPayments?:PhotoPayment[];photoDefaultAccountId?:string|null;movementDefaultAccountId?:string|null;urssafDefaultAccountId?:string|null;urssafStates?:UrssafState[];savingsProposals?:SavingsProposalDecision[];profiles?:SavingsProfile[];currentBalances?:Record<string,number>}){
 ...
 const points=useMemo<Point[]>(()=>{
  ...
  while(current<=horizon){const date=iso(current),monthKey=date.slice(0,7);
   ...
   movements.filter(m=>m.status==="planned"&&m.movement_date===date&&date>=todayIso).forEach(m=>{balances.set(m.account_id,(balances.get(m.account_id)??0)+(["income","transfer_in"].includes(m.movement_type)?Number(m.amount):-Number(m.amount)));if(m.movement_type==="expense")addExpense(m.category_id,Number(m.amount));});
 ...
 }),[accounts,points,todayIso,currentBalances]);
 ...
 const monthBudgets=budgetRoots.filter(b=>isBudgetActiveForMonth(b,month)&&(accountId==="all"||budgetTarget(b)===accountId)).map(b=>{const spent=monthOps.filter(o=>o.movement_type==="expense"&&rootFor(o.category_id)===b.id).reduce((s,o)=>s+Number(o.amount),0);return {...b,spent,remaining:Math.max(0,Number(b.monthly_budget)-spent)};});
 ...
 const simulationOperations=useMemo(()=>{if(!simulationEnabled||!simulationAccountId)return [] as {date:string;amount:number}[];const rows:{date:string;amount:number}[]=[];if(simulationType==="cash"){if(simulationAmount>0)rows.push({date:simulationDate,amount:simulationAmount});return rows;}if(simulationDownPayment>0)rows.push({date:simulationDate,amount:simulationDownPayment});if(simulationMonthlyPayment>0&&simulationMonths>0){let d=parse(simulationFirstPaymentDate);for(let i=0;i<simulationMonths;i++){rows.push({date:iso(d),amount:simulationMonthlyPayment});d=new Date(d);d.setMonth(d.getMonth()+1);}}return rows;},[simulationEnabled,simulationType,simulationAccountId,simulationDate,simulationAmount,simulationDownPayment,simulationMonthlyPayment,simulationMonths,simulationFirstPaymentDate]);
 ...
 const simulationSeries=useMemo(()=>{
  const firstOperationDate=simulationOperations.map(operation=>operation.date).sort()[0]??todayIso;
  const analysisEndDate=(()=>{const d=parse(firstOperationDate);d.setDate(d.getDate()+44);return iso(d);})();
  const sourceId=simulationAccountId||null;
  const profile=simulationProfile;
  const configuredSavingsId=profile?.destinationAccountId??null;
  const fundingSavingsId=simulationSavingsAccountId||null;
  const threshold=Math.max(0,Number(profile?.threshold??0));
  const emptyDetails=points.map(()=>({savingsBalance:fundingSavingsId?Number(currentBalances[fundingSavingsId]??0):0,thresholdGap:0}));
  if(!simulationEnabled||simulationOperations.length===0||points.length===0||!sourceId||!profile||!configuredSavingsId){
   return {points,details:emptyDetails,savingsTransferAmount:0,savingsBalanceAfterTransfer:fundingSavingsId?Number(currentBalances[fundingSavingsId]??0):0};
  }

  // Le scénario de référence contient déjà toutes les recommandations d'épargne.
  // On relance le même moteur avec l'achat ajouté comme mouvement prévu : le
  // moteur peut d'abord diminuer/annuler un versement d'épargne proposé et ne
  // demander une utilisation supplémentaire d'épargne que si cela reste utile.
  const syntheticMovements=simulationOperations.map((operation,index)=>({
   id:`simulation-${index}-${operation.date}`,
   account_id:sourceId,
   category_id:null,
   movement_type:"expense",
   amount:Math.max(0,Number(operation.amount)),
   movement_date:operation.date,
   status:"planned",
   recurrence_id:null,
   transfer_group_id:null,
  }));
  const simulatedRows=calculateSavingsPlan({
   sourceAccountId:sourceId,
   destinationAccountId:configuredSavingsId,
   initialChecking:Number(currentBalances[sourceId]??0),
   initialSavings:Number(currentBalances[configuredSavingsId]??0),
   startMonth:todayIso.slice(0,7),
   accounts,categories,
   movements:[...movements,...syntheticMovements],
   recurrences,overrides,exclusions,photoPayments,photoDefaultAccountId,
   urssafStates,urssafDefaultAccountId,movementDefaultAccountId,savingsProposals,
   months:60,minReserve:threshold,
   primaryIncomeCategoryId:profile.primaryIncomeCategoryId,
   primaryIncomeSource:profile.primaryIncomeSource,
   proposalTiming:profile.proposalTiming,
  });

  type RecommendationEffect={date:string;accountId:string;amount:number;kind:"deposit"|"use"};
  const effectsFromRows=(rows:typeof simulatedRows)=>{
   const effects:RecommendationEffect[]=[];
   for(const row of rows){
    if(row.proposal>0){
     const date=row.proposalDate??`${row.month}-28`;
     effects.push({date,accountId:sourceId,amount:-Number(row.proposal),kind:"deposit"});
     effects.push({date,accountId:configuredSavingsId,amount:Number(row.proposal),kind:"deposit"});
    }
    if(row.savingsUsed>0){
     const date=row.savingsUseDate??row.lowestBalanceDate??`${row.month}-28`;
     effects.push({date,accountId:configuredSavingsId,amount:-Number(row.savingsUsed),kind:"use"});
     effects.push({date,accountId:sourceId,amount:Number(row.savingsUsed),kind:"use"});
    }
   }
   return effects;
  };
  const simulatedEffects=effectsFromRows(simulatedRows);
  // Pour la référence on reprend les opérations virtuelles réellement présentes
  // dans la projection actuelle, afin de respecter aussi les décisions déjà
  // acceptées/supprimées et les deux passages mensuels éventuels.
  const baselineEffects:RecommendationEffect[]=virtualSavingsOps
   .filter(operation=>operation.savingsProposal&&(
    (operation.savingsProposal.sourceAccountId===sourceId&&operation.savingsProposal.destinationAccountId===configuredSavingsId)||
    (operation.savingsProposal.sourceAccountId===configuredSavingsId&&operation.savingsProposal.destinationAccountId===sourceId)
   ))
   .map(operation=>({
    date:operation.movement_date,
    accountId:operation.account_id,
    amount:["income","transfer_in"].includes(operation.movement_type)?Number(operation.amount):-Number(operation.amount),
    kind:operation.savingsProposal!.kind,
   }));

  const useCreditInWindow=(effects:RecommendationEffect[])=>effects
   .filter(effect=>effect.kind==="use"&&effect.accountId===sourceId&&effect.amount>0&&effect.date>=firstOperationDate&&effect.date<=analysisEndDate)
   .reduce((sum,effect)=>sum+effect.amount,0);
  const baselineUse=useCreditInWindow(baselineEffects);
  const simulatedUse=useCreditInWindow(simulatedEffects);
  const totalSimulationCost=simulationOperations.reduce((sum,operation)=>sum+Math.max(0,Number(operation.amount)),0);
  const incrementalUse=Math.min(totalSimulationCost,Math.max(0,simulatedUse-baselineUse));

  const fundingAtStart=fundingSavingsId?Number((points.find(point=>point.date>=firstOperationDate)?.balances[fundingSavingsId])??currentBalances[fundingSavingsId]??0):0;
  const availableFunding=fundingSavingsId?Math.max(0,fundingAtStart-SAVINGS_FLOOR):0;
  const savingsTransferAmount=fundingSavingsId?Math.min(incrementalUse,availableFunding):0;

  // Recalcul du graphique : la projection normale reste la base. On applique
  // l'achat, la variation des VERSEMENTS d'épargne proposés, puis uniquement le
  // complément d'utilisation d'épargne rendu nécessaire par l'achat.
  const deltaByDate=new Map<string,Map<string,number>>();
  const addDelta=(date:string,accountId:string,amount:number)=>{
   const byAccount=deltaByDate.get(date)??new Map<string,number>();
   byAccount.set(accountId,(byAccount.get(accountId)??0)+amount);
   deltaByDate.set(date,byAccount);
  };
  for(const effect of simulatedEffects.filter(effect=>effect.kind==="deposit"))addDelta(effect.date,effect.accountId,effect.amount);
  for(const effect of baselineEffects.filter(effect=>effect.kind==="deposit"))addDelta(effect.date,effect.accountId,-effect.amount);

  // Date du complément : première date où le plan recalculé demande davantage
  // d'épargne que le plan normal, dans la fenêtre de 45 jours.
  let extraUseDate=firstOperationDate;
  let cumulativeBaselineUse=0,cumulativeSimulatedUse=0;
  const useDates=Array.from(new Set([
   ...baselineEffects.filter(effect=>effect.kind==="use").map(effect=>effect.date),
   ...simulatedEffects.filter(effect=>effect.kind==="use").map(effect=>effect.date),
  ])).filter(date=>date>=firstOperationDate&&date<=analysisEndDate).sort();
  for(const date of useDates){
   cumulativeBaselineUse+=baselineEffects.filter(effect=>effect.kind==="use"&&effect.accountId===sourceId&&effect.amount>0&&effect.date===date).reduce((sum,effect)=>sum+effect.amount,0);
   cumulativeSimulatedUse+=simulatedEffects.filter(effect=>effect.kind==="use"&&effect.accountId===sourceId&&effect.amount>0&&effect.date===date).reduce((sum,effect)=>sum+effect.amount,0);
   if(cumulativeSimulatedUse>cumulativeBaselineUse+0.009){extraUseDate=date;break;}
  }
  if(savingsTransferAmount>0&&fundingSavingsId){
   addDelta(extraUseDate,fundingSavingsId,-savingsTransferAmount);
   addDelta(extraUseDate,sourceId,savingsTransferAmount);
  }

  const purchaseByDate=new Map<string,number>();
  for(const operation of simulationOperations)purchaseByDate.set(operation.date,(purchaseByDate.get(operation.date)??0)+Math.max(0,Number(operation.amount)));
  const cumulativeDeltas=new Map<string,number>();
  let cumulativePurchase=0;
  const simulated:Point[]=[];
  const details:{savingsBalance:number;thresholdGap:number}[]=[];
  for(const baseline of points){
   if(baseline.date>=firstOperationDate)cumulativePurchase+=purchaseByDate.get(baseline.date)??0;
   const dayDelta=deltaByDate.get(baseline.date);
   if(dayDelta)for(const [id,amount] of dayDelta)cumulativeDeltas.set(id,(cumulativeDeltas.get(id)??0)+amount);
   const balances={...baseline.balances};
   if(baseline.date>=firstOperationDate)balances[sourceId]=Number(balances[sourceId]??0)-cumulativePurchase;
   for(const [id,amount] of cumulativeDeltas)balances[id]=Number(balances[id]??0)+amount;
   const checking=accounts.filter(account=>account.account_type==="checking").reduce((sum,account)=>sum+Number(balances[account.id]??0),0);
   const savings=accounts.filter(account=>account.account_type==="savings").reduce((sum,account)=>sum+Number(balances[account.id]??0),0);
   const baselineSource=Number(baseline.balances[sourceId]??0);
   const simulatedSource=Number(balances[sourceId]??0);
   const thresholdGap=baseline.date>=firstOperationDate&&baseline.date<=analysisEndDate?Math.max(0,Math.max(0,threshold-simulatedSource)-Math.max(0,threshold-baselineSource)):0;
   simulated.push({...baseline,balances,checking,savings,total:checking+savings});
   details.push({savingsBalance:fundingSavingsId?Number(balances[fundingSavingsId]??0):0,thresholdGap});
  }
  const balanceDisplayDate=savingsTransferAmount>0?extraUseDate:firstOperationDate;
  const balancePoint=simulated.find(point=>point.date>=balanceDisplayDate)??simulated.at(-1);
  const savingsBalanceAfterTransfer=fundingSavingsId?Number(balancePoint?.balances[fundingSavingsId]??fundingAtStart):0;
  return {points:simulated,details,savingsTransferAmount,savingsBalanceAfterTransfer};
 },[points,simulationEnabled,simulationOperations,simulationAccountId,simulationSavingsAccountId,simulationProfile,accounts,currentBalances,todayIso,categories,movements,recurrences,overrides,exclusions,photoPayments,photoDefaultAccountId,urssafStates,urssafDefaultAccountId,movementDefaultAccountId,savingsProposals,virtualSavingsOps]);
...
function ProjectionChart({values,baselineValues,cursor,color}:{values:number[];baselineValues?:number[];cursor:number;color?:string|null}){
 if(values.length<2)return null;

 // Conserver les extrema de chaque tranche évite de masquer un minimum ou un
 // maximum de courte durée sur les projections longues. La courbe de référence
 // du simulateur reste affichée en pointillés lorsqu'une simulation est active.
 const maxBuckets=180;
 const bucketSize=Math.max(1,Math.ceil(values.length/maxBuckets));
 const sampledIndexes=(series:number[])=>{
  const selectedIndexes=new Set<number>([0,series.length-1]);
  for(let start=0;start<series.length;start+=bucketSize){
   ...
   selectedIndexes.add(minIndex);selectedIndexes.add(maxIndex);
  }
  return [...selectedIndexes].sort((a,b)=>a-b).map(index=>({index,value:series[index]}));
 };
 const sample=sampledIndexes(values);
 const allValues=baselineValues?.length?[...values,...baselineValues]:values;
 ...
 const baselinePts=baselineValues?.length?sampledIndexes(baselineValues).map(point=>`${(point.index/Math.max(1,baselineValues.length-1))*100},${yFor(point.value)}`).join(" "):null;
 ...
 return <div className="mt-6 overflow-hidden rounded-2xl p-3" style={{backgroundColor:color?`${color}14`:"#fafafa",borderLeft:color?`5px solid ${color}`:undefined}}>
  <svg viewBox="0 0 100 100" className="h-52 w-full" preserveAspectRatio="none" role="img" aria-label="Évolution du solde projeté avec repères tous les mille euros et seuil zéro en rouge">
   {axisMax>0?<rect x="0" y="10" width="100" height={Math.max(0,yFor(0)-10)} fill="#16a34a" opacity=".035"/>:null}
   {axisMin<0?<rect x="0" y={yFor(0)} width="100" height={Math.max(0,90-yFor(0))} fill="#dc2626" opacity=".055"/>:null}
   ...
   {baselinePts?<polyline points={baselinePts} fill="none" stroke="currentColor" strokeWidth="1.2" opacity=".28" strokeDasharray="3 2" vectorEffect="non-scaling-stroke"/>:null}
   <polyline points={pts} fill="none" stroke={color??"currentColor"} strokeWidth="1.8" vectorEffect="non-scaling-stroke"/>
